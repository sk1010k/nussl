...
class EvaluationBase(object):
    ...
    Args:
        true_sources_list (list): List of objects that contain one ground truth source per object.
            In some instances (such as the BSSEval-objects) this list is filled with
            :class:`audio_signal.AudioSignals` but in other cases it is populated with
            :class:`separation.masks.mask_base.MaskBase` -derived objects (i.e., either a
            :class:`separation.masks.binary_mask.BinaryMask` or
            :class:`separation.masks.soft_mask.SoftMask` object).
        estimated_sources_list (list): List of objects that contain source estimations from a
            source separation algorithm. List should be populated with the same type of
            objects and in the same order as ``true_sources_list``.
        source_labels (list): List of strings that are labels for each source to be used as keys for
            the scores. Default value is `None` and in that case labels are
            `Source 0`, `Source 1`, etc.
        do_mono (bool): Whether to make the objects in ``true_sources_list`` and
            ``estimated_sources_list`` mono prior to doing the evaluation. This may not work
            for all :class:`EvaluationBase`-derived objects.
    ...
    def __init__(self, true_sources_list, estimated_sources_list, source_labels=None, **kwargs):
        ...
        if len(self.true_sources_list) != len(self.estimated_sources_list):
            raise ValueError('Must have the same number of objects in true_sources_list '
                             'and estimated_sources_list!')

        # set the labels up correctly
        if source_labels is None:
            self.source_labels = ['Source {}'.format(i) for i in range(len(self.true_sources_list))]
        else:
            assert isinstance(source_labels, list), 'Expected source_labels to be a ' \
                                                    'list of strings!'
    ...
    def _verify_input_list(audio_signal_list):
        """
        Base method for verifying a list of input objects for an
        :class:`EvaluationBase`-derived object. Override this method when creating new
        :class:`EvaluationBased`-derived class.
        
        By default calls :func:`nussl.utils.verify_audio_signal_list_strict`, which verifies
        that all objects in the input list are :class:`audio_signal.AudioSignal` objects with
        the same length, sample rate and have identical number of channels.
        
        Args:
            audio_signal_list (list): List of objects that contain one signal per object.
                In some instances (such as the BSSEval-objects) this list is filled with
                :ref:`AudioSignals` but in other cases it is populated with
                :class:`separation.masks.mask_base.MaskBase` -derived objects (i.e., either a
                :class:`separation.masks.binary_mask.BinaryMask` or
                :class:`separation.masks.soft_mask.SoftMask` object). In the latter case,
                this method is overridden with a specific function in
                :class:`evaluation.precision_recall_fscore.PrecisionRecallFScore`.
